		...
		<div class="header">
			<div class="right-week">
				<picker @change="changeWeek" :value="weekIndex" :range="weekList">
					<view class="uni-input">{{weekList[weekIndex]}}<uni-icons color='#fff' type="down"
							size="22"></uni-icons>
					</view>
				</picker>
			</div>
			...
			<div class="content-main">
				...
				<div class="right-course-list">
					<div v-for="data in weekCourseList" class="course-list one">
						<course-plus-item :type='currentShowType.value' :data='d' v-for="d in data.courseList"
							:key="d.id"></course-plus-item>
					</div>
				</div>
			</div>
		</div>
...
<script setup>
	...
	function gropCourseList() {
		function _isSame(d, c) {
			return d.courseName === c.courseName && (sectionSortIndex[d.courseSection] - sectionSortIndex[c
				.courseSection]) === -1
		}
		weekCourseList.value.forEach(d => {
			let prevCourse = null
	...
	onMounted(() => {
		initWeekList()
		indexData.value.className = uni.getStorageSync("home-active-type") === "student" ? uni.getStorageSync(
