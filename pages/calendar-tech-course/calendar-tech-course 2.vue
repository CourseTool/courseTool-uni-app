<template>
	<view class="calendar-tech-course" v-if="isShowPage">
		<div @click="isShowSelectTech = true" class="header">
			<p style="text-decoration: underline;">当前教师:{{indexData.teacher}}更改</p>
		</div>
		...
		<div class="course-list">
			...
			<div v-if="courseDataList.length !== 0" class="course-item" v-for="d in courseDataList.courseList"
				:key="d.id">
...
<script setup>
	...
	onMounted(async () => {
		indexData.teacher = uni.getStorageSync('home-active-type') === "teacher" ? uni.getStorageSync(
			"home-active-val") : "吴宇飞"
		const date = new Date()
