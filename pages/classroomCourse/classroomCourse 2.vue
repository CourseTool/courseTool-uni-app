		...
		<div class="course-tools">
			<div class="class">
				<div class="title">
					...
					<span @click="isShowSelectClassroom=true">{{indexData.classroom}}<span
							style="text-decoration: underline;">(更改)</span></span>
					<span style='text-decoration: underline;' @click="gotoCurrentDay">今天</span>
				</div>
				<select-class type="classroom" @hide-select-class="hiddenSelectClassroom"
					v-if="isShowSelectClassroom"></select-class>
			</div>
			<div class="week-day">
				<courseDate-weekDay :hasCourseList='hasCourseWeekDayList' @changeWeekDay="changeWeekDay"
					:date="currentDate" v-model="indexData.weekDay"></courseDate-weekDay>
			</div>
			<courseDate-week ref="weekComponent" @selectDate="selectDate" @changeWeek="changeWeek"
				v-model="indexData.week"></courseDate-week>
		</div>
		<div class="course-content">
			<div class="empty" v-if="!courseData.length">
				<p>该教室今日无课</p>
			</div>
			<div v-else class="course-list">
				<!-- <courseItem :courseData='data' v-for='data in courseData.courseList' :key='data.id'></courseItem> -->
				<div class="course-item" v-for='data in courseData' :key='data.id'>
					<div class="title">
...
<script setup>
	...
	import {
		...
		getClassroomWeekHasCourseDayListAPI
	} from '@/API/course.js'
	import {
		ref,
		onMounted,
	...
	async function getCourseData() {
		...
		const {
			data
		} = await getClassroomCourseAPI(indexData)
		courseData.value = data.data
		if (!courseData.value) courseData.value = []
